            ...
            if (e <= tpos && e >= b && e > fpos) {
                ...
                if (!display_end.empty()) {
                    if (!display_end_attrs.empty()) {
                        events.push_back (pos_event(e, e == b ? (priority +1002) : (-priority -1 -nest), substitute_attr_values(display_end, &display_end_attrs, &num)));
                    } else {
                        events.push_back (pos_event(e, e == b ? (priority +1002) : (-priority -1 -nest), display_end));
                    }
                }
                if (!display_class.empty() && e > b)
...
bool KWICLines::nextline () {
    ...
    Labels::iterator end = lab.find (Concordance::lngroup_labidx);
    for (Labels::iterator i = lab.upper_bound(0); i != end; ++i) {
        Position p = i->second;
        if (p == -1) continue;
    ...
    for (vector<pos_event>::iterator e = events.begin(); 
        ...
        case ev_END:
            if (rightctx->chars) {
                int maxchars = rightctx->chars;
                VSit beg = partstr.begin(), end = partstr.end();
                for (; beg != end; ++beg) {
                    if (maxchars <= 0) {
                        parttag.erase (beg - partstr.begin() + parttag.begin(),
                                       parttag.end());
                        partstr.erase (beg, end);
                        break;
                    }
                    if (inutf8)
                        maxchars -= utf8len((*beg).c_str());
                    else
                        maxchars -= (*beg).size();
                }
            }
        ...
        case ev_BKW:
            ...
            tags.insert ("coll");
            if (leftctx->chars) {
                int maxchars = leftctx->chars;
                VSit beg = partstr.begin(), end = partstr.end();
                while (beg != end) {
                    if (maxchars <= 0) {
                        parttag.erase (parttag.begin(),
                                       end - partstr.end() + parttag.end());
                        partstr.erase (beg, end);
                        break;
                    }
                    --end;
                    if (inutf8)
                        maxchars -= utf8len((*end).c_str());
                    else
                        maxchars -= (*end).size();
                }
