...
void CorpusManager::run (istream &in, ostream &out)
{
	...
	try {
		...
		string grps;
		getline (args, grps);
		c->delete_linegroups (grps.c_str());
	    } else if (cmd == "subcorp") {
		if (subcorpdir) {
		    string subcorpname, corpname, structname, query;
		    args >> subcorpname >> corpname >> structname;
		    getline (args, query);
		    query += ';';
		    Corpus *c = get_corp(corpname);
		    Structure *struc = c->get_struct (structname);
		    try {
			FastStream *s = eval_cqponepos (query.c_str(), struc);
			if (s->peek() == s->final()) {
			    lasterr = "empty subcorpus";
			} else {
			    SubCorpus *sub = 
				create_subcorpus (subcorpdir, corpname, 
						  subcorpname, s, c, 
						  struc->rng);
			    corpora [corpname + ':' + subcorpname] = 
				CIC ("", sub);
			    delete s;
			    lasterr = "";
			}
		...
		getline (args, regpat);
		trimspaces (regpat);
		PosAttr *a = findPosAttr (c->conf, attrname);
		...
		if (canrebuild (cname)) {
		    ...
		    system (("encodevert -c " + cname + " 2>&1").c_str());

		}
	    } else if (cmd == "version") {
		out << MANATEE_VERSION_STR << '\n';
	    } else if (cmd == "exit") {
		out << "\v\n" << flush;
		return;
	    } else {
