# ===========================================================================
#          http://www.nongnu.org/autoconf-archive/ax_ruby_devel.html
# ===========================================================================
#
# SYNOPSIS
#
...
AC_DEFUN([AX_RUBY_DEVEL],[
    ...
    AX_PROG_RUBY_VERSION("1.9", [RUBY_CONFIG="RbConfig"], [RUBY_CONFIG="Config"])
...
$ac_mkmf_result. The Ruby bindings won't be built.])
	    ...
	    if test -z "$RUBY_CPPFLAGS"; then
	        ruby_path=`$RUBY -rmkmf -e "print $RUBY_CONFIG::CONFIG[[\"rubyhdrdir\"]]"`
	        if test -n "${ruby_path}"; then
	                ruby_path="-I$ruby_path"
	        fi
	        RUBY_CPPFLAGS=$ruby_path
	        ruby_arch=`$RUBY -rmkmf -e "print $RUBY_CONFIG::CONFIG[[\"arch\"]]"`
	        if test -n "${ruby_arch}"; then
	            RUBY_CPPFLAGS="$ruby_path $ruby_path/$ruby_arch"
	        fi
	        ruby_archdir=`$RUBY -rmkmf -e "print $RUBY_CONFIG::CONFIG[[\"archdir\"]]"`
	        if test -n "${ruby_archdir}"; then
	            RUBY_CPPFLAGS="$ruby_path $ruby_path/$ruby_arch -I$ruby_archdir"
	        fi
	    fi
	    ...
	    if test -z "$RUBY_LDFLAGS"; then
	        RUBY_LDFLAGS=`$RUBY -rmkmf -e "print $RUBY_CONFIG::CONFIG[[\"LIBRUBYARG_SHARED\"]]"`
	    fi
	    ...
	    if test -z "$RUBY_SITE_PKG"; then
	        RUBY_SITE_PKG=`$RUBY -rmkmf -e "print $RUBY_CONFIG::CONFIG[[\"vendorarchdir\"]]"`
	    fi
	    ...
	    if test -z "$RUBY_EXTRA_LIBS"; then
	       RUBY_EXTRA_LIBS=`$RUBY -rmkmf -e "print $RUBY_CONFIG::CONFIG[[\"SOLIBS\"]]"`
	    fi
	    ...
	    # RUBY_EXTRA_LDFLAGS=`$RUBY -rmkmf -e "print $RUBY_CONFIG::CONFIG[[\"LINKFORSHARED\"]]"`
	    # fi
